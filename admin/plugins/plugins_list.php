...
?>
<b><?=ucwords(str_replace("_"," ","plugins"))?></b>
  <table cellspacing="3" cellpadding="3" border="0"  width="100%" class="bdr">
			  ...
			  <form name="search_frm" id="search_frm" method="post">
				<table width="60%" border="0"  cellpadding="0" cellspacing="0" class="bodytext">
				  <TR>
					<TD  nowrap="nowrap">
					  <?php
						  $hash    =  getTableFieldsName("plugins");
					...
					<td nowrap="nowrap" align="right">
					  <input type="hidden" name="cmd" id="cmd" value="search_plugins" >
   ...
   <tr>
   <td> 
		<!--<a href="plugins.php?cmd=edit" class="nav3">Add a plugin</a>-->
        
        <?php if($message) echo "<p>$message</p>"; ?>
        <form enctype="multipart/form-data" method="post" action="">
            <label>Choose a zip file to upload: <input type="file" name="zip_file" /></label>
            <br />
            <input type="hidden" name="cmd" value="upload_plugin" />
            <input type="submit" name="submit" value="Upload" />
        </form>
       <br /><br />
        Example <br />
         Make a folder & Write in a file : <br />
        <code>
        /*<br />
		  Name: Photos <br />
		  Author: Amirul Momenin <br />
		  Version: 1.0 <br />
		*/<br />
	   </code>
		<table cellspacing="1" cellpadding="3" border="0" width="100%" class="bodytext">
		 ...
		 ?>
			<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
			  <td><?=$arr[$i]['plugin_name']?></td>
			  <td><?=$arr[$i]['plugin_description']?></td>
			  <td>
			      <?php
                       
					   if($arr[$i]['plugin_status']=="activate")
					   {
				 ?>
                        <a href="plugins.php?cmd=deactive&id=<?=$arr[$i]['id']?>" class="nav">Deactive</a> 
                 <?php	   
					   }
					   else
					   {
				?>
                      <a href="plugins.php?cmd=activate&id=<?=$arr[$i]['id']?>" class="nav">Activate</a> 
                <?php	   
					   }
				  ?>
              </td>
			  <td><?=$arr[$i]['uploaded_path']?></td>
              <td><?=$arr[$i]['activated_path']?></td>
			  <td><?=$arr[$i]['date_installed']?></td>			  
			  <td nowrap >
				  <a href="plugins.php?cmd=edit&id=<?=$arr[$i]['id']?>" class="nav">Edit</a> |
				  <a href="plugins.php?cmd=delete&id=<?=$arr[$i]['id']?>" class="nav" onClick=" return confirm('Are you sure to delete this item ?');">Delete</a> 
			 </td>
