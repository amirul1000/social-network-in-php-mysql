...
<b><?=ucwords(str_replace("_"," ","shared"))?></b><br />
<table cellspacing="3" cellpadding="3" border="0" align="center" width="98%" class="bdr">
 <tr>
  <td>  
     <a href="shared.php?cmd=list" class="nav3">List</a>
	 <form name="frm_shared" method="post"  enctype="multipart/form-data" onSubmit="return checkRequired();">			
            ...
            <tr>
              ...
              <tr>
                     ...
                     <td><?php
                            ...
                            $resusers  =  $db->select($info);
                        ?>
                        <select  name="shared_users_id" id="shared_users_id"   class="textbox">
                            <option value="">--Select--</option>
                            <?php
                               foreach($resusers as $key=>$each)
                               { 
                            ?>
                              <option value="<?=$resusers[$key]['id']?>" <?php if($resusers[$key]['id']==$shared_users_id){ echo "selected"; }?>><?=$resusers[$key]['first_name']?> <?=$resusers[$key]['last_name']?></option>
                            <?php
                             }
                            ?> 
                        </select>
