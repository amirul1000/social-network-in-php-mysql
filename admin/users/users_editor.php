	 ...
	 <form name="frm_users" method="post"  enctype="multipart/form-data" onSubmit="return checkRequired();">			
		<table cellspacing="3" cellpadding="3" border="0" align="center" class="bodytext" width="100%">  
		 <tr>
						 ...
						 <td>Date Of Birth</td>
						 <td>
						    <input type="text" name="date_of_birth" id="date_of_birth"  value="<?=$date_of_birth?>" class="textbox">
							<a href="javascript:void(0);" onclick="displayDatePicker('date_of_birth');"><img src="../../images/calendar.gif" width="16" height="16" border="0" /></a>
						 </td>
				     </tr><tr>
		           		 <td>Gender</td>
				   		 <td><?php
...
<select  name="country_id" id="country_id"   class="textbox">
	...
	?> 
						 ...
						 <td>
						    <input type="text" name="verification_code" id="verification_code"  value="<?=$verification_code?>" class="textbox">
						 </td>
				     </tr><tr>
		           		 <td>Verified</td>
				   		 <td><?php
	$enumusers = getEnumFieldValues('users','verified');
?>
<select  name="verified" id="verified"   class="textbox">
<?php
   foreach($enumusers as $key=>$value)
   { 
?>
  <option value="<?=$value?>" <?php if($value==$verified){ echo "selected"; }?>><?=$value?></option>
 <?php
  }
?> 
</select></td>
