...
?>
<b><?=ucwords(str_replace("_"," ","likes"))?></b>
				...
				<table width="60%" border="0"  cellpadding="0" cellspacing="0" class="bodytext">
				  <TR>
					...
					<td nowrap="nowrap" align="right">
					  <input type="hidden" name="cmd" id="cmd" value="search_likes" >
			...
			<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
			  ...
			  <td>
		                            <?php
										...
										$info2['table']    = users;	
										$info2['fields']   = array("password");	   	   
										$info2['where']    =  "1 AND id='".$arr[$i]['likes_users_id']."' LIMIT 0,1";
										$res2  =  $db->select($info2);
										echo $res2[0]['password'];	
					                ?>
							   </td>
			  <td>
		                            <?php
										...
										$info2['fields']   = array("content_type");	   	   
										$info2['where']    =  "1 AND id='".$arr[$i]['contents_id']."' LIMIT 0,1";
										$res2  =  $db->select($info2);
										echo $res2[0]['content_type'];	
					                ?>
							   </td>
			  <td><?=$arr[$i]['like_count']?></td>
			  <td><?=$arr[$i]['date_created']?></td>
			  
			  <td nowrap >
				  <a href="likes.php?cmd=edit&id=<?=$arr[$i]['id']?>" class="nav">Edit</a> |
				  <a href="likes.php?cmd=delete&id=<?=$arr[$i]['id']?>" class="nav" onClick=" return confirm('Are you sure to delete this item ?');">Delete</a> 
			 </td>
